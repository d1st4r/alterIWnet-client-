...
static void PatchMW2_ClientConsole_AutoComplete()
{
	...
	*(BYTE*)0x5A3E9F = 22; // jump target if no files found
	...
	drawCmdMatchAlternativeHook.initialize(5, (PBYTE)0x5A3EB6);
	drawCmdMatchAlternativeHook.installHook(DrawCmdMatchAlternativeStub, true, false);

	// ConDrawInput_DetailedDvarMatch
	drawDvarMatchAlternativeHook.initialize(6, (PBYTE)0x5A3DC2);
	drawDvarMatchAlternativeHook.installHook(DrawDvarMatchAlternativeStub, true, false);

	// cmd autocompletion
	*(BYTE*)0x5A7367 = 80;

	completeCmdMatchAlternativeHook.initialize(5, (PBYTE)0x5A73B8);
	completeCmdMatchAlternativeHook.installHook(CompleteCmdMatchAlternativeStub, true, false);

	// dvar autocompletion
	*(BYTE*)0x5A73DD = 105;

	completeDvarMatchAlternativeHook.initialize(5, (PBYTE)0x5A7447);
	completeDvarMatchAlternativeHook.installHook(CompleteDvarMatchAlternativeStub, true, false);
}
