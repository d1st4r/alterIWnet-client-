...
{
	pingDisplayHook.initialize(5, (PBYTE)pingDisplayHookLoc);
	pingDisplayHook.installHook(PingDisplayHookFunc, true, false);

	hpongResultHook.initialize((PBYTE)hpongResultHookLoc);
	hpongResultHook.installHook(HpongResultHookFunc, false);

	retryGameSearchHook.initialize((PBYTE)retryGameSearchHookLoc);
	retryGameSearchHook.installHook(RetryGameSearchHookFunc, false);

	qosHook.initialize((PBYTE)qosHookLoc);
	qosHook.installHook(QosHookFunc, false);

	ping_searches_per = Dvar_RegisterInt("ping_searches_per", 2, 1, 30, 0, "Number of searches at each min ping value");
	ping_increment = Dvar_RegisterInt("ping_increment", 10, 1, 500, 0, "Number of milliseconds to increase min ping after each set of searches");
	ping_default_min = Dvar_RegisterInt("ping_default_min", 50, 10, 500, 0, "Minimum ping for the initial set of searches");
}
