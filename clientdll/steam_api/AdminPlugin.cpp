...
namespace aIW
{
	void AdminPluginCode::Initialize()
	{
		...
		Log::Initialize("aIWExt.log", LogLevel::Debug | LogLevel::Info | LogLevel::Warning | LogLevel::Error, true);
	...
	{
		SayAll(gcnew String(aiw_sayName->current.string), message);
	}
	...
	{
		SayTo(clientNum, gcnew String(aiw_sayName->current.string), message);
	}
...
{
	return aIW::AdminPluginCode::TriggerSay(entity, name, message);
}

void APC_TriggerFrame()
{
	return aIW::AdminPluginCode::TriggerFrame();
}

void AP_FlushRedirect(char* outputbuf)
{
	aIW::AdminPluginBase::RedirectThis(outputbuf);
}
#endif